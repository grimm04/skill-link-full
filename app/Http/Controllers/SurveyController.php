...
class SurveyController extends AppBaseController
{
    ...
    public function home($email, UserSurvey $userSurvey)
    { 
        $userSurveys = $userSurvey->where('email', $email)->first();
    ...
    public function survey_subscription(Request $request)
    {
        $email1 = Session::get('email');
        return view('surveys.subscription')
            ->with('email1', $email1);
    }
