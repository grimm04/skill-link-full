	...
	<div class="row">
		<div class="col-md-9">
			...
			<div class="wrapper" style="margin-bottom: 20px;">
				<div class="row">
					<div class="col-md-12">
						<form method="post" id="form-medic" action="{{ route('profile_post_addmedical') }}">
						{!! csrf_field() !!}
												...
												<div class="row">
													<div class="col-xs-5 col-sm-4 col-md-4">
														<div style="margin-bottom: 10px;">
															<span>Condition</span><span style="color: red;">*</span>
															<input type="text" name="condition[][condition]" class="form-control">
														</div>
													</div>
													<div class="col-xs-5 col-sm-4 col-md-4">
														<div style="margin-bottom: 10px;">
													...
													<div class="col-xs-5 col-sm-4 col-md-4">
														<div style="margin-bottom: 10px;">
															<span>From</span>
															<input type="date" name="from[][from]" class="form-control" value="{{ \Carbon\Carbon::now()->format('Y-m-d')}}">
														</div>
													</div>
												</div>
											</div>
						...
						@if (count($interest) == 0)
							<div class="box border-all-radius" style="margin-bottom: 15px;">
								<form method="post" id="form-add-ex" action="{{ route('profile_post_addinterests') }}">
								{!! csrf_field() !!}
								<div class="box-header with-border">
						...
						@if (count($duty) == 0) 
							<div class="box border-all-radius" style="margin-bottom: 15px;">
								<form method="post" id="form-add-ex" action="{{ route('profile_post_addfitduties') }}">
								{!! csrf_field() !!}
								<div class="box-header with-border">
								...
								<div style="margin-bottom: 10px;">
									...
									@foreach ($value->fit as $fit)
										<label class="input-checkbox">
						                  <p>{{ $fit->description}}</p> 
					                  		<input type="{{ $fit->type }}" {{ $fit->type == 'checkbox' ? 'name=dutyid_checkbox[]' : 'name=dutyid_'. $fit->dutyid .'' }}  id="dutyid-{{ $fit->id }}" value="{{ $fit->id }}" onclick="fitduty(this)" >
